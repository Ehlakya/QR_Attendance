...
import { Search, Loader2, User, BookOpen, AlertCircle, CheckCircle2, Calendar, Users, TrendingUp, X, Download, FileText, TableProperties } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-textPrimary">
            6-Month Attendance Analytics
          </h2>
          <p className="text-textSecondary mt-1 font-medium">Search and analyze student attendance history or view global stats</p>
        </div>
      </div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <Card className="border-border bg-card shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-semibold text-textSecondary mb-1">Present Today</p>
                          <h3 className="text-3xl font-bold text-success">{defaultOverview.overview.presentTodayPercent}%</h3>
                        </div>
                  ...
                  <Card className="border-border bg-card shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-semibold text-textSecondary mb-1">Absent Today</p>
                          <h3 className="text-3xl font-bold text-danger">{defaultOverview.overview.absentTodayPercent}%</h3>
                        </div>
                  ...
                  <Card className="border-border bg-card shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-semibold text-textSecondary mb-1">Avg Monthly</p>
                ...
                <Card className="border-border bg-card shadow-sm overflow-hidden">
                  <CardHeader className="border-b border-border">
                    <CardTitle>Student Leaderboard (Top 50)</CardTitle>
                  </CardHeader>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-background/50 text-textSecondary uppercase text-xs font-bold border-b border-border">
                        <tr>
                          ...
                          <th className="px-6 py-4">Last 3 Months</th>
                      ...
                      <tbody className="divide-y divide-border">
                        {defaultOverview.tableData.map((row) => (
                          <tr key={row.id} className="hover:bg-background/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-textSecondary">{row.registerNumber}</td>
                            <td className="px-6 py-4 font-medium text-textPrimary">{row.className}</td>
                            <td className="px-6 py-4">
                              <div className="flex items-end gap-2 h-8">
                                {row.monthWise.slice(-3).map((m, i) => (
                                  <div key={i} className="group relative flex flex-col justify-end h-full w-4 bg-background border border-border rounded-t-sm">
                                    <div 
                                      className={`w-full rounded-t-sm ${m.percentage >= 75 ? 'bg-success' : 'bg-danger'}`} 
                                      style={{ height: `${m.percentage}%` }}
                                    ></div>
                                    <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-card text-textPrimary text-xs font-bold px-2 py-1 rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-10 whitespace-nowrap">
